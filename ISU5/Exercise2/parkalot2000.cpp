...
void * plcs_entryguard(void *) {
	while (run) {
		...
		while (!carWaitingEntry)
			pthread_cond_wait(&cond_entry, &mutex_entry);		// I WANT IN
		doorEntryOpen = true;
		pthread_cond_signal(&cond_entry);						// You can get in
		while (carWaitingEntry)
			pthread_cond_wait(&cond_entry, &mutex_entry);		// Car is in
		doorEntryOpen = false;
		pthread_mutex_lock(&counter_mut);
		currentCars++;
		cout << "Number of cars: " << currentCars << endl;
